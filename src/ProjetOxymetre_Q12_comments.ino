...
Objectif : 
  ...
  - Menu1() : Affichage du BPM et clignotement LED au rythme du pouls
  - Menu2() : Afficher les données du capteur brutes pouvant être lues sur le traceur série
  - Menu3() : Afficher le BPM et le Sp02 sur le moniteur série 
...
////// pour le bouton
int Mode = 1; // Par défaut mode 1
const int PauseAntiReb = 80; // Pause anti rebond pour l'appui du bouton
enum {LECTURE_BOUTON, DETECTION_APPUI_BOUTON, ATTENTE_RELACHEMENT_BOUTON} etat; // Différents états
int etatBouton;
bool etatBoutonSauvegarde = LOW;
...
void setup() {
  ...
  pinMode(ButtonPin, INPUT); // Initialisation de la pin du bouton
...
//////////////////////////////////// CHOIX DU MENU ////////////////////////////////////////
  ...
  switch (etat) {
    ...
    case LECTURE_BOUTON: // On regarde si on appuie ou non sur le bouton
      etatBouton = digitalRead(ButtonPin);
      if (etatBouton == HIGH) { // bouton appuyé
          Serial.println("Appui détecté");
          departChrono = millis(); // départ anti-rebond
          etat = DETECTION_APPUI_BOUTON;
      }
      break;

    case DETECTION_APPUI_BOUTON: // Si on a appuyé sur le bouton, on s'assure de ne pas prendre en compte les rebonds
      if (millis() - departChrono > PauseAntiReb) {
        // changement de mode une seule fois
        switch (Mode) { // Selon le Mode dans lequel on était on passe au mode suivant
          case 1: Mode = 2; break;
          case 2: Mode = 3; break;
          case 3: Mode = 1; break;
        }
        ...
        Serial.println(Mode);
        etat = ATTENTE_RELACHEMENT_BOUTON; // attendre que le bouton soit relâché
      }
      break;

    case ATTENTE_RELACHEMENT_BOUTON: // Quand le bouton est relaché on repasse en mode lecture
      etatBouton = digitalRead(ButtonPin);
      if (etatBouton == LOW) { // bouton relâché
          etat = LECTURE_BOUTON;
      }
      break;
  }
...
// Affichage du BPM et clignotement LED au rythme du pouls
void Menu1(){
  unsigned long currentMillis = millis();
  if (currentMillis - previousMillis_bpm >= interval_bpm) {
    previousMillis_bpm = currentMillis;

    Serial.print("compteur_bpm : ");
    Serial.println(4* compteur_bpm);
...
// Afficher les données du capteur brutes pouvant être lues sur le traceur série
void Menu2(){ // dans ce menu on affiche les données brutes
  Serial.print("Led R: ");
  Serial.print(moyenne_R);
  Serial.println(" | AC_R_filtre: ");
  Serial.println(AC_R_filtre);
  Serial.print("Led IR: ");
  Serial.print(moyenne_IR);
  Serial.println(" | AC_IR_filtre: ");
  Serial.println(AC_IR_filtre);
}


// Afficher le BPM et le Sp02 sur le moniteur série 
void Menu3(){ // dans ce menu on affiche le taux d'oxygène et le bpm
  //SPO2(); la fonction que Nolan aura codé
  unsigned long currentMillis = millis();
  if (currentMillis - previousMillis_bpm >= interval_bpm) {
    previousMillis_bpm = currentMillis;

    Serial.println("compteur_bpm : ");
    Serial.println(4* compteur_bpm);
    compteur_bpm = 0; // Réinitialisation
  }
  if (!battement_detecte && AC_IR_filtre > seuil_h) {

    // Si un battement est détecté, on calcule le SpO2
  ...
  // Si on a déjà detecté un battement et qu'on passe sous le seuil bas on passe à faux
...
double calculSpO2(double ratio) {
  ...
  return 108.0 - 20.0 * ratio - 0.375 * (ratio * ratio);
}
...
void LedV_pulse() {
  ...
  if (!battement_detecte && AC_IR_filtre > seuil_h) {
    ...
    // Si un battement est détecté, on allume la LED verte
    digitalWrite(LedVPin, HIGH);
    compteur_bpm++;
    battement_detecte = true; // on a détecté un battement
